...
class Model {
    ...
    function M($table_name = '', $db_type = 'mysql' , $connection='')
    {
        if( $table_name == '' ){ return false; }
        if( $db_type == 'mysql' ){
            return $this->MC( $table_name ,'', $connection);
        }else if( $db_type == 'mongo' ){
            $db_prefix = C('DB_PREFIX');
            $db_config['DB_TYPE'] = "mongo";
            $db_config['DB_USER'] = C('MONGO_USER');
            $db_config['DB_PWD']  = C('MONGO_PWD');
            $db_config['DB_HOST'] = C('MONGO_HOST');
            $db_config['DB_PORT'] = C('MONGO_PORT');
            $db_config['DB_NAME'] = C('MONGO_DB_NAME');
            
            return $this->MC( 'Think\\Model\\MongoModel:' . $table_name , $db_prefix , $db_config );
        }else{
            return false;
        }
    }
