...
//token公式
function token_calculation()
{
    $token_str = date("Y-m",time()).$_SERVER['HTTP_HOST'].date("-d",time()).session('token');
    $token = md5($token_str);
    return $token;
}
...
function chk_token(){
    ...
    if(I('post.token/s') == token_calculation()){
        $result = true;
    }
...
//mail_title 标题
//mail_content 正文
function mailto($mailto,$mail_title,$mail_content)
{
...
/*
    列队管理
 */
function Q($name,$value = '',$options=null){
    static $cache   =   '';
    ...
    if($value == ''){
        return $cache->mq_out($name);
    }else{
        return $cache->mq_in($name, $value);
    }
}
/**
 * 缓存管理
